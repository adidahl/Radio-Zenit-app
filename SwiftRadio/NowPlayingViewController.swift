...
class NowPlayingViewController: UIViewController {
    ...
    func stationDidChange() {
        ...
        updateLabels("Ucitavanje stanice...")
    ...
    @IBAction func pausePressed() {
        ...
        radioPlayer.pause()
        updateLabels("Stanica pauzirana...")
    ...
    func queryAlbumArt() {
        ...
        let ArtistNamee = track.artist
        let range = ArtistNamee.startIndex.advancedBy(7)..<ArtistNamee.endIndex
        let substringg = ArtistNamee[range]
        
        let SongName = self.track.title
        let rangeOfDomain = SongName.startIndex..<SongName.endIndex.advancedBy(-4)
        let SongNameNew = SongName[rangeOfDomain]
        
        
        if useLastFM {
           queryURL = String(format: "http://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=%@&artist=%@&track=%@&format=json", apiKey, substringg, SongNameNew)
        } else {
            queryURL = String(format: "https://itunes.apple.com/search?term=%@+%@&entity=song", substringg, SongNameNew)
        }
        ...
        if(radioPlayer.timedMetadata != nil && radioPlayer.timedMetadata.count > 0)
        {
            ...
            dispatch_async(dispatch_get_main_queue()) {
                ...
                if currentSongName != self.track.title {
                    ...
                    //I added this code because my label has too many blank spaces and could not insert artist strings correctly
                    let ArtistName = self.track.artist
                    let range = ArtistName.startIndex.advancedBy(7)..<ArtistName.endIndex
                    let substring = ArtistName[range]
                    ...
                    if DEBUG_LOG {
                        print("METADATA artist: \(substring) | title: \(SongNameNew)")
                    }
                    
                    // Update Labels
                    //self.artistLabel.text = self.track.artist
                    self.songLabel.text = SongNameNew
                    self.artistLabel.text = substring
